...
SET csv_file_path = log_file_path + '\housekeeping_linkfile_purge_missing_type' 
   + REPLACE(REPLACE(REPLACE(LEFT(CAST(xtimestamp AS VARCHAR), 16), '-', ''), ':', ''), ' ', '')
   + '.csv';
SET log_file_path = log_file_path + '\housekeeping_linkfile_purge_missing_type' 
   + REPLACE(REPLACE(REPLACE(LEFT(CAST(xtimestamp AS VARCHAR), 16), '-', ''), ':', ''), ' ', '')
   + '.log';
...
-- Start log file
EXECUTE ('xp_cmdshell ''echo ' || CAST(GETDATE() AS VARCHAR(23)) || ': Starting Linkfile Purge Missing Type >> "' || log_file_path || '"''');
