...
END IF;
IF VAREXISTS('type_include_list') = 0 THEN
  CREATE VARIABLE type_include_list VARCHAR(900);
END IF;
...
SET choke_limit = 2;
SET create_months_older_than = 12;
SET update_months_older_than = 12;
SET type_include_list = 'WPCV,WPC1,WPC2,WPC3,WPC4';
...
SET csv_file_path = log_file_path + '\housekeeping_linkfile_purge_previous_cv' 
   + REPLACE(REPLACE(REPLACE(LEFT(CAST(xtimestamp AS VARCHAR), 16), '-', ''), ':', ''), ' ', '')
   + '.csv';
SET log_file_path = log_file_path + '\housekeeping_linkfile_purge_previous_cv' 
   + REPLACE(REPLACE(REPLACE(LEFT(CAST(xtimestamp AS VARCHAR), 16), '-', ''), ':', ''), ' ', '')
   + '.log';
...
EXECUTE ('xp_cmdshell ''echo ' || CAST(GETDATE() AS VARCHAR(23)) || ': Variable TYPE_EXCULUDE_LIST =^> '|| type_include_list || ' >> "' || log_file_path || '"''');
...
WHERE l.parent_object_name IN(''person'')
  ...
  AND l.type IN(''' + REPLACE(type_include_list, ',', ''',''') + ''')
...
DROP VARIABLE type_include_list;
